...
async function getLessonData(phaseNumber: number, moduleSlug: string, lessonSlug: string, userId: string) {
  ...
  return {
    ...
    exerciseData,
    basePath: `/mba/${phaseNumber}/${moduleSlug}`,
  };
}
...
export default async function LessonPage({
  ...
  params: Promise<{ phaseSlug: string; moduleSlug: string; lessonSlug: string }>;
}) {
  const { phaseSlug, moduleSlug, lessonSlug } = await params;
  ...
  return (
    <SimpleLessonViewer
      ...
      breadcrumbs={[
        ...
        { label: "MBA Curriculum", href: "/mba" },
        { label: `Phase ${data.phase.number}`, href: `/mba` },
        { label: data.module.name, href: `/mba/${phaseSlug}/${moduleSlug}` },
      ]}
    />
  );
}
