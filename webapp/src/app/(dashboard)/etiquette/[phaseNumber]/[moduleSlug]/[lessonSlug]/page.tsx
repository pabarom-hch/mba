...
async function getLessonData(phaseNumber: number, moduleSlug: string, lessonSlug: string, userId: string) {
  ...
  return {
    ...
    exerciseData,
    basePath: `/etiquette/${phaseNumber}/${moduleSlug}`,
  };
}

export default async function EtiquetteLessonPage({
  params,
}: {
  ...
  return (
    <SimpleLessonViewer
      ...
      breadcrumbs={[
        ...
        { label: "Elite Etiquette", href: "/etiquette" },
        { label: `Phase ${data.phase.number}`, href: `/etiquette` },
        { label: data.module.name, href: `/etiquette/${phaseSlug}/${moduleSlug}` },
      ]}
    />
  );
}
