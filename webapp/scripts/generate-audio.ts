...
/**
 * Get audio duration from MP3 buffer (approximate)
 */
function estimateAudioDuration(buffer: Buffer, textLength: number): number {
...
async function main() {
  ...
  for (let i = 0; i < lessons.length; i++) {
    ...
    try {
      ...
      const durationSeconds = estimateAudioDuration(audioBuffer, charCount);
